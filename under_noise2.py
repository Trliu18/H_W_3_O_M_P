# under noise OMP know l2 norm of noise
import os
...
if os.path.exists('figure/under_noise2'):
    print('under_noise2 folder exists')
else:
    os.makedirs('figure/under_noise2')
    ...
    for N in [20, 50, 100]:
        ...
        for M in Ms:
            for n in noise_range:
                res_list = repeated_expermients(s, M, N, noise_sigma=n, know_s=False)
        ...
        file_path = "under_noise2_{}.npy".format(N)
        np.save("under_noise2_{}.npy".format(N), transition_map)
        img_name = 'under_noise2_{}'.format(N)
        ...
        plt.title('Noise, know l-norm, N = {}'.format(N))
        plt.colorbar()
        plt.savefig('figure/under_noise2/{}.png'.format(img_name))
